...
def register(request):
    if request.method == "POST":
        ...
        if user_form.is_valid()*userprofile_form.is_valid():
            ...
            new_profile.save()
            return render(request,'account/pageJump.html')
        else:
...
def user_login(request):
    if request.method == "POST":
        ...
        if login_form.is_valid():
            ...
            if user:
                login(request, user)
                return  redirect(reverse('account:gotohome'))
            else:
                return render(request,'core/pageJump.html',)
        else:
            # 取出验证失败的信息,方便定位问题,给用用户提示
            ErrorDict = login_form.errors
